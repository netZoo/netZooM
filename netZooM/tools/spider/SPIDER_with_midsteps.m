function RegNet=PANDA(RegNet, GeneCoReg, TFCoop, alpha, savetag, savesteps);
...
if(ismember(-1, savesteps))
	save([savetag, '_Step', num2str(-1), '.mat'], 'RegNet', 'GeneCoReg', 'TFCoop', '-v7.3');
end
...
hamming=1;
while(hamming>0.001)
	if(ismember(step, savesteps))
		save([savetag, '_Step', num2str(step), '.mat'], 'RegNet', 'GeneCoReg', 'TFCoop', '-v7.3');
	end
...
if(max(savesteps)>step)
	save([savetag, '_Final.mat'], 'RegNet', 'GeneCoReg', 'TFCoop', '-v7.3');
end
