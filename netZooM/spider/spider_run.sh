...
nohup matlab -nodisplay -nosplash -nodesktop -nojvm -r "run('spider_config.m'); run('spider_run.m'); quit;" >& panda.`hostname`.log

# Email notification when done
echo "SPIDER run on `hostname` has just finished: `date`." | mail -s "Task finished on `hostname`" -a panda.`hostname`.log `whoami`
